...
public class GestureController : MonoBehaviour {
    ...
    void updatePalm(Hand_ENUM hand)
    {
        ...
        if (leftHandPalm != null & rightHandPalm != null)
        {
            ...
            {
                if (leftHand.PalmPosition.DistanceTo(rightHand.PalmPosition) < 150.0f)
                {
                    spellMergeBall();
                }
            }
    ...
    void spellMergeBall()
    {
        if ((leftHandPalm.hasBall && rightHandPalm.hasBall) && (!leftHandPalm.hasMergedBall && !rightHandPalm.hasMergedBall))
        {
            ...
            if (!tele)
                StartCoroutine(DelaySpellBlink(2.0f));
    ...
    IEnumerator DelaySpellBlink(float f)
    {
        tele = true;
